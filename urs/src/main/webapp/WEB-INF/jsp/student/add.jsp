...
<div class="row justify-content-center">
	<div class="card border-width-3 border-radius-0 border-color-hover-dark mb-4 col-lg-6">
		<div class="card-body">
		<h4 class="font-weight-bold text-uppercase text-4 mb-3">학생 정보 등록</h4>
			<form action="${request.getContextPath()}/student" method="post" class="needs-validation" onsubmit="return check(this);">
				<div class="form-group row">
					<label class="col-lg-3 font-weight-bold text-dark col-form-label form-control-label text-2 required">학번</label>
					<div class="col-lg-9">
				...
				<div class="col-lg-10">
					<a href="/student"><input type="button" value="취소" class="btn btn-primary btn-modern float-right" /></a>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	function check(form) {
			...
			if (xhr.readyState === xhr.DONE) {
				if (xhr.status === 200 || xhr.status === 201) {
					...
					if (row === null) {
						...
						if (row.memberNo != 0) {
							document.getElementById("errorMemberNo").innerHTML = "<font size=\"2em\" color=\"red\">이미 존재하는 학번입니다.</font>";
						} 
						if (row.cardNo != null) {
		...
		xhr.open("GET", "http://localhost/student/check/" + memberNo + "/" + cardNo, true);
		xhr.send();
